...
unsafe fn get_process_name(handle:*mut c_void) -> String{
...
fn parse_process_name(name:&str)->Option<String>{
...
#[cfg(windows)]
pub unsafe  fn get_current_active_process_name() -> String{
    ...
    unsafe{
        ...
        let name = get_process_name(process_handle);
        parse_process_name(&name).unwrap_or(name)
